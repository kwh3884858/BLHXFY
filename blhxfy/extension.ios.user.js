...
(function () {
	...
	const getLocalConfig = () => {
	  ...
	  const keys = ['autoDownload', 'bottomToolbar', 'displayName', 'removeScroller', 'hideSidebar', 'originText', 'storyOnly', 'showTranslator', 'transJa', 'transEn', 'font', 'transApi', 'fontBold', 'plainText', 'battleTrans', 'log', 'defaultFont'];
	  keys.forEach(key => {
  ...
  `;
	...
	function autoDownloadCsv () {
	  if (config.autoDownload) {
	    ...
	    $('#wrapper').off('click.blhxfy-dlcsv2').on('click.blhxfy-dlcsv2', '.pop-synopsis .btn-usual-ok,.btn-scene-skip', function () {
	      if (!downloaded) {
	        win.blhxfy.sendEvent('dlStoryCsv');
	      }
	    });
